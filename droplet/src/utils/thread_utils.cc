...
[[nodiscard]] std::string GetNativeThreadName() noexcept {
  try {
    ...
    if (pthread_getname_np(pthread_self(), native_name.data(),
                           native_name.size()) != 0) {
      return {};
    }
...
void SetThreadName(std::string name) {
  if (name.empty()) {
    ...
    t_thread_name = native_name.empty() ? std::string(DEFAULT_THREAD_NAME)
...
std::string_view GetThreadName() noexcept { return t_thread_name; }
