...
#include <droplet/utils/string_utils.h>
...
[[nodiscard]] bool DecodeUtf8CodePoint(std::string_view text,
                                       std::size_t& offset,
    ...
    for (std::size_t i = 0; i < text.size(); ++i) {
      ...
      if constexpr (sizeof(wchar_t) == 2) {
        if (IsHighSurrogate(code_point)) {
          ++i;
          ASSERT_RETVAL2(i < text.size(), {},
                         "宽字符串包含不完整的 UTF-16 代理对");
          const u32 low_surrogate = static_cast<u32>(text[i]);
          ASSERT_RETVAL2(IsLowSurrogate(low_surrogate), {},
                         "宽字符串包含非法 UTF-16 代理对");
          code_point = 0x10000U + ((code_point - HIGH_SURROGATE_FIRST) << 10U) +
        ...
        ASSERT_RETVAL2(code_point <= MAX_UNICODE_CODE_POINT &&
                           !IsHighSurrogate(code_point) &&
                           !IsLowSurrogate(code_point),
                       {}, "宽字符串包含非法 UTF-32 码点");
      }
    ...
    while (offset < text.size()) {
      ...
      if constexpr (sizeof(wchar_t) == 2) {
        if (code_point <= 0xFFFF) {
          ...
          code_point -= 0x10000;
          result.push_back(
              static_cast<wchar_t>(HIGH_SURROGATE_FIRST + (code_point >> 10U)));
          result.push_back(static_cast<wchar_t>(LOW_SURROGATE_FIRST +
                                                (code_point & 0x3FFU)));
        }
      } else {
        result.push_back(static_cast<wchar_t>(code_point));
