...
LogLevel::Level Logger::getLevel() const noexcept { return impl_->getLevel(); }

std::string_view Logger::getName() const noexcept { return impl_->getName(); }
...
void Logger::addAppender(AppenderPtr appender) {
  impl_->addAppender(std::move(appender));
}
...
void Logger::clearAppenders() { impl_->clearAppenders(); }

void Logger::flush() { impl_->flush(); }

void Logger::sync() { impl_->sync(); }

// Logger::Impl

Logger::Impl::Impl(std::string name) : name_(std::move(name)) {
  if (name_.empty()) {
...
void Logger::Impl::submit(const detail::LogRecordView& record) noexcept {
  ...
  try {
    ...
    for (const AppenderPtr& appender : appenders_) {
      detail::AppenderAccess::Append(appender, record.level_,
                                     formatted_record.view());
    }
  }
...
std::string_view Logger::Impl::getName() const noexcept { return name_; }

void Logger::Impl::setFormatter(
    std::shared_ptr<const detail::Formatter> formatter) {
  ASSERT_RETNONE2(formatter != nullptr, "logger formatter cannot be null");
  std::unique_lock<std::shared_mutex> lock(mutex_);
  formatter_ = std::move(formatter);
}

std::shared_ptr<const detail::Formatter> Logger::Impl::getFormatter()
    const noexcept {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  return formatter_;
}
...
void Logger::Impl::setRoot(const LoggerPtr& root) noexcept { root_ = root; }

void detail::LoggerAccess::Submit(Logger& logger,
                                  const LogRecordView& record) noexcept {
  logger.impl_->submit(record);
}

void detail::LoggerAccess::SetRoot(Logger& logger,
...
class LoggerManager final : public Singleton<LoggerManager> {
 public:
  [[nodiscard]] LoggerPtr getRoot() const noexcept { return root_; }
 ...
 private:
  LoggerManager() : root_(std::make_shared<Logger>("root")) {
...
LoggerManager& GetLoggerManager() { return LoggerManager::GetInstance(); }

LoggerPtr GetRootLogger() { return GetLoggerManager().getRoot(); }
