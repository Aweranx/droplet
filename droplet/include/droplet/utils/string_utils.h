 ...
 */
class DROPLET_API StringUtils final {
 public:
   ...
   */
  [[nodiscard]] static std::string WStringToString(
      std::wstring_view text) noexcept;

  /**
   * @brief UTF8字符串往宽字符转换
   * 在Windows平台上，wchar_t宽字符是UTF-16,
   * 在Linux平台上，wchar_t宽字符是UTF-32
   * @param text
   * @return std::wstring 转换失败时触发断言，返回空字符串
   */
  [[nodiscard]] static std::wstring StringToWString(
